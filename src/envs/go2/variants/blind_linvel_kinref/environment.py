...
class Go2Env:
    ...
    def __init__(
        ...
        Args:
            ...
            step_k: Half-period of the trot phase clock in control steps.
            gait_scale: Amplitude of the kinematic trot reference.
        ...
        self.gait_ref = make_kinematic_ref(step_k, scale=gait_scale, dt=self.dt)
        self.cycle_len = self.gait_ref.shape[0]
        ...
        dummy_info = {
            "step": jp.array(0),
            "phase_offset": jp.array(0, dtype=jp.int32),
        ...
        print(f"  action_scale={action_scale}, gait_scale={gait_scale}")
        print(
    ...
    def reset(self, rng: jax.Array, difficulty: jax.Array = jp.array(0.0)) -> EnvState:
        ...
        phase_offset = jax.random.randint(keys[5], (), 0, self.cycle_len)
    ...
    def step(self, state: EnvState, action: jax.Array) -> EnvState:
        ...
        phase = (state.info["step"] + state.info["phase_offset"]) % self.cycle_len
        ref = self.gait_ref[phase]
        ctrl = self.default_joints + ref + action * self.action_scale
        ...
        next_phase_offset = jp.where(
            done, reset_state.info["phase_offset"], state.info["phase_offset"]
        )
        ...
        next_info = {
            "step": next_step,
            "phase_offset": next_phase_offset,
    ...
    def _compute_rewards(
        ...
        correction = data.qpos[7:] - self.default_joints - ref
    ...
    def compute_reward_from_critic_obs(
        ...
        phase_sin = critic_obs[..., 49]
        phase_cos = critic_obs[..., 50]
        phase_angle = jp.mod(jp.arctan2(phase_sin, phase_cos), 2 * jp.pi)
        phase = jp.mod(
            jp.floor(phase_angle / (2 * jp.pi) * self.cycle_len + 0.5).astype(jp.int32),
            self.cycle_len,
        )
        ref = self.gait_ref[phase]
    ...
    def _get_actor_obs(self, data, info) -> jax.Array:
        ...
        This variant exposes local linear velocity and gait phase to the actor.
        """
        quat = data.qpos[3:7]
        inv_q = quat_inv(quat)
        phase = 2 * jp.pi * (
            (info["step"] + info["phase_offset"]) % self.cycle_len
        ) / self.cycle_len
    ...
    def _get_obs(self, data, info) -> jax.Array:
        ...
        return jp.concatenate(
            [
                ...
                jp.array([jp.sin(phase), jp.cos(phase)]),  # Gait phase (2)
            ]
        )  # Total: 51
